...
这是**验收工具**，不是跑分工具。它回答的是「这块盘有没有骗我」，不是「这块盘跑分多少」。

| 设备类型 | 支持 | 说明 |
|---|:---:|---|
| 移动固态硬盘 (PSSD) | ✅ | 主要目标，全部功能可用 |
| U 盘 | ✅ | **扩容盘重灾区，最该测的就是这类** |
| SD / microSD（经读卡器） | ✅ | 同上，假卡极多 |
| 移动机械硬盘 | ✅ | 容量校验有效；机械盘没有 SLC 缓存，会显示「未见拐点」 |
| 内置副盘（D: 等） | ✅ | 可用 |
| 系统盘 | ❌ | 代码主动拒绝 |
| RAW / 未格式化盘 | ❌ | 依赖写文件工作，需先有文件系统 |

块大小按容量自适应，小到 128MB 的 U 盘也能拿到足够采样点：
...
- **4K 随机读写 / IOPS / 延迟** —— 只做顺序读写。要看这些请用 CrystalDiskMark。
- **耐久度 / TBW 测试** —— 那需要反复写入几百 TB。
- **细粒度坏道扫描** —— 哨兵间隔为块大小的 1/16，抓的是 MB/GB 级地址回绕，不是单个坏扇区。
...
**测试数据的生成速度不能成为瓶颈。** 每块现场生成伪随机数据只有约 40MB/s，比被测硬盘还慢，测出来的就成了 CPU 性能。本工具一次性生成 16MiB 随机基块后平铺复用，每块只改写块头和哨兵，生成成本趋近于零。

**测试数据必须不可压缩。** 否则带透明压缩的主控可以少写实际数据，虚报速度。基块是真随机的，每个 4K 扇区都不可压缩。

**光看块头不够。** 若扩容盘的回绕粒度大于块大小，块头可能恰好躲过覆盖。所以每块还散布 16 个哨兵，把检测粒度降到块大小的 1/16（1GiB 块即 64MiB）。
...
```bash
python test_verify.py      # 77 项回归测试
```

测试用例覆盖块生成唯一性、扩容盘回绕检测、中段覆盖、截断、自适应块大小（128MB～4TB）、SLC 拐点识别、桥接芯片识别和判定逻辑。改动核心逻辑后务必跑一遍。
...
- **不检测随机单比特翻转**——那类错误由固态自身的 ECC 负责，本工具针对的是 MB/GB 级的地址回绕。
